...
static void ShortcutEditDialog_OnOK(HWND hDlg)
{
	...
	if (sc == 0) {
		::MessageBox(hDlg, _T("请设置一个快捷键。"), NULL, MB_OK | MB_ICONEXCLAMATION);
		return;
	}
	BYTE key = LOBYTE(sc);
	BYTE mod = HIBYTE(sc);
	...
	if (!(g_scEdit && scShortcutKey == g_scShortcutKey)) {
		...
		if (!unique) {
			::MessageBox(hDlg, _T("这个快捷键已经被使用。"), NULL, MB_OK | MB_ICONEXCLAMATION);
			return;
		}
	}
	...
	if (scPath[0] == 0) {
		::MessageBox(hDlg, _T("请选择一个记录文件。"), NULL, MB_OK | MB_ICONEXCLAMATION);
		return;
	}
	if (!::PathFileExists(scPath)) {
		::MessageBox(hDlg, _T("请选择一个存在的记录文件。"), NULL, MB_OK | MB_ICONEXCLAMATION);
		return;
	}
