...
static const SLVCOLUMN s_listColumns[] = {
	...
	{ LVCFMT_RIGHT,  60, _T("过去50战"), SLVSORT_INTEGEREX }
};
...
exception:
	...
	MessageBox(hDlg, _T("战绩更新失败。"), NULL, MB_OK | MB_ICONSTOP);
}
...
static LRESULT ScoreLineView_OnRDoubleClick(HWND hParent, HWND hwnd)
{
	...
	if (ListView_SubItemHitTest(hwnd, &hti) != -1) {
		...
		if (hti.iSubItem == 2 || hti.iSubItem == 3) {
			...
			if (!fltCustom) {
				::MessageBox(NULL, _T("战绩更新失败。"), NULL, MB_OK);
				return FALSE;
			}
			SCORELINE_FILTER_DESC filterDesc = *fltCustom;
			BOOL trDirLeft = reinterpret_cast<BOOL>(GetProp(hParent, PROP_TRDIRLEFT));
	...
	for (int i = 1; i < SCORELINE_FILTER__MAX; i <<= 1) {
		switch(fltCustom.mask & i) {
			...
			case SCORELINE_FILTER__TIMESTAMP_END:
				...
				title += 
					Formatter(_T("到%d/%02d/%02d为止"), 
						loctime.wYear, loctime.wMonth, loctime.wDay);
				break;
			case SCORELINE_FILTER__LIMIT:
				title += Formatter(_T("最近%d战"), fltCustom.limit);
				break;
		}
	}
...
{
	int itemIndex = 0;
	::InsertMenu(hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_REFLESH, _T("刷新"));
	::InsertMenu(hSysMenu, itemIndex++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	::InsertMenu(hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_UNDOSCR, _T("撤销"));
	::InsertMenu(hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_REDOSCR, _T("重做"));
	::InsertMenu(hSysMenu, itemIndex++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	::InsertMenu(hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_RNKPROF, _T("对手列表"));
	::InsertMenu(hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_TRKRECD, _T("对战记录查询"));
	::InsertMenu(hSysMenu, itemIndex++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	::InsertMenu(hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_CLIPBRD, _T("复制到剪贴板"));
	::InsertMenu(hSysMenu, itemIndex++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
}
