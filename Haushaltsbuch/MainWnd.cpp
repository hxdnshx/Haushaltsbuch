...
static BOOL MainWindow_OnCreate(HWND hwnd, LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!ScoreLine_Open(createProfile)) {
		...
		do {
			::MessageBox(hwnd, _T("档案映射失败。"), NULL, MB_OK | MB_ICONSTOP);
...
static void NotifyMenu_OnAutorunReportToolOpenFileName(HWND hwnd)
{
	...
	Autorun_OpenFileName(hwnd, path, _T("对战记录上传工具(ProfileUploader.exe)\0ProfileUploader.exe\0对战记录上传工具 (kkb_report.exe)\0kkb_report.exe\0"), _T("ReportTool"));
}
