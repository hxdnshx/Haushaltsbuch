...
static const SLVCOLUMN s_listColumns[] = {
	{ LVCFMT_LEFT,   80, _T("名字"),   SLVSORT_LPARAM },
	{ LVCFMT_RIGHT,  55, _T("对战总数"), SLVSORT_INTEGER },
	{ LVCFMT_RIGHT,  55, _T("胜"),     SLVSORT_INTEGER },
	{ LVCFMT_RIGHT,  55, _T("负"),     SLVSORT_INTEGER },
	{ LVCFMT_RIGHT,  55, _T("胜率"),   SLVSORT_INTEGER },
	{ LVCFMT_RIGHT,  60, _T("最近50"), SLVSORT_INTEGEREX }
};
...
exception:
	...
	::MessageBox(hDlg, _T("战绩刷新失败。"), NULL, MB_OK | MB_ICONSTOP);
}
...
static BOOL ScoreLineQIBDialog_InitListView(HWND hDlg)
{
	...
	item.pszText = _T("最近30战"); item.iItem++; LVPARAMFIELD(item.lParam).charId++; 
	ListView_InsertItem(listWnd, &item);
	item.pszText = _T("最近50战"); item.iItem++; LVPARAMFIELD(item.lParam).charId++; 
	ListView_InsertItem(listWnd, &item);
	item.pszText = _T("最近100战"); item.iItem++; LVPARAMFIELD(item.lParam).charId++;
	ListView_InsertItem(listWnd, &item);
...
static void ScoreLineQIBDialog_OpenProfile(HWND hDlg, LPCTSTR fileName)
{
	...
	if (ScoreLine_Open(true)) {
		...
		MessageBox(hDlg, _T("记录文件打开失败。"), NULL, MB_OK | MB_ICONSTOP);
	}
}
...
{
	int itemIndex = 0;
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_OPENHIT, _T("打开其他对战记录"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION | MF_CHECKED, UC_DIRP1, _T("按自己使用的角色分"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_DIRP2, _T("按对手使用的角色分"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION | MF_CHECKED, UC_VIEWALL, _T("全部对战"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_VIEWTOD, _T("今天的对战"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_VIEWYES, _T("昨天的对战"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_VIEWCST, _T("自定义筛选条件"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_UNDOSCR, _T("撤销"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_REDOSCR, _T("重做"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_RNKPROF, _T("对手列表"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_TRKRECD, _T("对战记录查询"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
	::InsertMenu(s_hSysMenu, itemIndex++, MF_STRING | MF_BYPOSITION, UC_CLIPBRD, _T("复制到剪贴板"));
	::InsertMenu(s_hSysMenu, itemIndex++, MF_SEPARATOR | MF_BYPOSITION, 0, NULL);
}
